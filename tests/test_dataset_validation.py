...
def test_map_native_format_fails_closed_without_bastion_native_validation(tmp_path: Path) -> None:
    (tmp_path / "base.gpkg").write_bytes(b"SQLite format 3\x00")
    with pytest.raises(DatasetValidationError, match="Bastion GDAL validation"):
        validate_map_dataset(tmp_path, source_id="map-provider", license_id="license")
